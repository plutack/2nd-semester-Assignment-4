  ...
  */
  try {
    ...
    if (newBook.newBooksArray && authorIndex !== -1) {
      authorDB[authorIndex].books = authorDB[authorIndex].books.concat(
        newBook.newBooksArray,
      );
      await writeDB(authorDB);
      res.status(200).json({
        success: true,
        author: authorDB[authorIndex],
        message: `${newBook.newBooksArray.length} books added successfully`,
      });
    }
  } catch (error) {
